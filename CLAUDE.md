...
| #102 | Concluir migração react-query: auditoria de código (jul/2026) achou **56 candidatos reais** (bem mais que a estimativa antiga de ~24-35) + 16 write-only (só trocar callback por `invalidateQueries`) + 12 hooks de segurança/monitoramento excluídos por design. **Lotes 1+2 migrados (28 arquivos)**: alertas operacionais/financeiros/custo BOM, permissões (HR/clientes/usuários/módulos), Cotações completo, checklist de produção, BOM de material e composto, ficha técnica (ações), calendário/detalhe/checklist de eventos, `useConfig`/`useTaxonomy` (usados em ~15 arquivos — cache único agora compartilhado), detalhe de cliente, precificação — **~28 restantes**. Migrar o resto ao tocar em cada área. Ver memória `react-query-data-fetching-standard`. | Média |
