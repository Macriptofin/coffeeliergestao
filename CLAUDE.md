...
- **Baixa manual (Contas a Receber → Receber / Contas a Pagar → Pagar) — set/2026**: o front só insere `receipt_transactions`/`payment_transactions`; saldo, status, data de baixa, caixa e saldo bancário são 100% dos triggers (`update_receivable_remaining_amount` agora considera desconto/juros lançados **no recibo**; `update_payable_remaining_amount` carimba `payment_date`). Não existe status `'Parcial'` no banco (CHECK só aceita Pendente/Recebido|Pago/Vencido/Cancelado) — "Parcial" é derivado na tela quando há baixa parcial. **Formas de pagamento = cadastro `payment_methods`** (o CHECK com lista fixa em `receipt_transactions.receipt_method` foi removido; era a causa do "Erro ao registrar recebimento" com "Transferência Bancária"). No DRE, a "Classificação da Diferença" do recibo vira: Desconto de Antecipação / Taxa Bancária → despesa financeira; Desconto Comercial → dedução da receita; Juros/Multa → receita financeira (competência = data do recebimento). **Antecipação Monkey de NF única**: registrar o recebimento pelo **líquido** + deságio no recibo (Desconto de Antecipação); o crédito da financeira casa por valor na conciliação. Lote com N NFs (ou NF única) direto na Conciliação: natureza **Baixa de contas a receber** (kind `baixa_receber`, RPC `reconcile_settle_receivables`) → escolher as NFs → recibo por NF pelo saldo, deságio rateado, caixas vinculados à linha, desfazer reverte.
...
- **Acesso**: mesmo Supabase Auth; `user_profiles.user_type='client'` + `client_users (user_id, client_id, portal_role 'solicitante'|'aprovador')`. Convite pela aba Portal do cadastro do cliente (edge `invite-client-user`); redefinição `portal-reset-password`. **E-mail já interno não pode virar cliente** (perderia acesso interno).
  - **`client_contacts` (universo) × `client_users` (acesso concedido)**: nem todo contato do cliente vira usuário do portal — só quem efetivamente precisa (os solicitantes de verdade). O seletor "Contato / Solicitante" no `ProposalEditor.tsx` lista `client_contacts` (cadastro leve: nome + e-mail + departamento), não `client_users`. Ao clicar **"Enviar proposta"** (`handleSend`), o sistema cruza o e-mail do contato escolhido contra `client_users` já ativos; se não achar, **convida automaticamente nesse instante** via `invite-client-user`, usando nome/e-mail do contato — não é preciso passar pela aba Portal → Convidar antes. Essa aba continua existindo como caminho alternativo pra dar acesso sem estar mandando proposta junto. O cadastro que a pessoa preenche ao definir a senha é só a senha — não há hoje etapa de autopreenchimento de prédio/sala (isso é escolhido pela equipe por composição; ver item "Departamento self-service" no backlog, ainda não construído).
...
### Conciliação Bancária (set/2026)

**Financeiro → Conciliação Bancária** (`src/pages/financeiro/ConciliacaoBancaria.tsx`, rota `/financeiro/conciliacao`; parser `src/lib/ofx.ts`; migrations `20260905120000_bank_reconciliation.sql` e `20260905150000_bank_reconciliation_links.sql`). Objetivo: colocar as contas em dia **a partir do extrato** (OFX) em vez de lançamento manual. Fluxo: importar OFX (dedupe por `fitid`) → "Casar com lançamentos existentes" (`reconcile_auto_match`: mesmo valor ±R$ 0,01, mesma conta, ±5 dias) → "Sugerir por regras" (`reconciliation_rules`: padrão ILIKE em contraparte/descrição + direção; ~185 regras semeadas a partir das decisões do usuário) → classificar em lote (`reconcile_apply`) → desfazer por linha (`reconcile_undo`).

- **Tipos de linha** (`bank_statement_lines.kind`): `despesa` (cria AP paga + caixa), `receita` (AR recebida), `transferencia` (entre contas, casa a contraparte ±3 dias), `retirada_socio` (3.2), `aporte_socio` (3.3), `emprestimo_recebido`/`emprestimo_pago` (2.1.1), `repasse` (2.1.2 — valores de terceiros, ex.: cartão do gerente da CMPC via Tainara), `ignorar`, `existente` (casou com lançamento já existente), **`baixa_receber`** (10/09/2026, migration `20260910130000`: 1 crédito ↔ N NFs em aberto via `reconcile_settle_receivables(line, ar_ids[], adjustment_type, receipt_method)` — um recibo por NF pelo saldo em aberto, diferença rateada pelo saldo como deságio/desconto se o crédito é menor ou juros/multa se é maior, guarda de 25%, caixas vinculados em `bank_statement_line_links`; o desfazer apaga os recibos e o trigger devolve o saldo. Na tela, botão "Sugerir combinação" acha o subconjunto de NFs que fecha com o crédito. Nunca cria AR nova nem AP de deságio — o deságio fica no recibo e o DRE o lê como despesa financeira). Decisões do usuário: Pix do sócio = retirada; pró-labore = salário mínimo (1 Pix/mês em 5.2.1.4); compras no débito em mercado = 5.1.2 **sem estoque** (marco zero, inventário depois); maquininha = receita bruta + taxa 5.3.5; antecipação = AR pelo valor cheio + deságio 5.3.4; **CBRL Serviços Digitais** (Pix via WOOVI IP) = resgates da corretora de criptoativos do sócio (PF) → aporte 3.3 (não é InfinityPay nem receita; regra criada e 16 linhas classificadas em 05/09/2026).
- **Vínculo 1 linha ↔ N lançamentos** (`bank_statement_line_links` + RPC `reconcile_link_existing(line, cash_ids[], notes)`, soma assinada tem que bater ±0,02): necessário porque as financeiras da **Monkey** (BNP Paribas, Sofisa, Crefisa, Itaú — compram os títulos da CMPC) pagam em **lote por (banco, data de operação)** = Σ líquido das NFs, e a CMPC paga várias NFs num Pix só. A página mostra "N lançamentos em lote".
- **Reconstrução da receita 2026** (one-off arquivado em `supabase/one-off/20260905_reconc_rebuild_revenue.sql`; a função e as tabelas de apoio foram **removidas do banco** depois da gravação porque reexecutar duplicaria os recebimentos — só o `reconc_log` fica como trilha; rodava em modo ensaio lançando exceção com o resumo JSON): gravada em 05/09/2026 e **verificada por auditoria independente** (NF↔AR 122/122, Monkey 54/54, 147 vínculos em 69 linhas somando exato, saldo bancário íntegro) — 122 NFs válidas (NF-e VHSYS 226→293 + NFS-e Guaíba 1→60, NFS-e 9 = duplicata da 14; + NF-e 213→225 de dez/2025) viraram AR (22 casaram com AR manuais pré-existentes, mantendo a data do evento como `competence_date`; 100 novas); 54 antecipações Monkey = recebimento pelo valor cheio + AP de deságio (fornecedor = financeira); Pix de clientes casados por NF única, combinação (subset-sum) ou parcial. **Nunca voltar a um único `.limit()` na listagem**: PostgREST corta em 1.000 linhas — a página pagina com `.range()`.
- **Pendências desta frente** (set/2026): identidades de PF/PJ sem regra (ZOOP, PIX Marketplace, Jackeline, Eder, Luiza, Ação Produções, Amanda Freischlag, Roselene, Claudia Chacon, Noeli Brass, Jefferson Venâncio, Luciano Gurniak, Fernanda Biavatti, Geneci, Rodrigo Menezes, Maiquelem, Airton de Conti, Fabrício Bicca, Milene, Leonelo, Antonio Cabreira, Vedere/FNS); **44 AR em aberto antes do corte** (R$ 120.742,27: 17 manuais sem NF da CMPC = R$ 87.198,60, incl. "Kits Turnos da Virada 24 e 31/12" R$ 56.700 sem NF equivalente; 27 NF sem recebimento localizado = R$ 33.543,67, CMPC + PF/PJ que podem ter pago pela maquininha) + 1 AP CEEE fev/2026 R$ 2.265,95 — auditado em 10/09/2026: **nenhuma duplicidade provável por valor** nos dois sentidos; o usuário decidiu fazer pente-fino manual caso a caso, **nada cancelado** (40 delas têm PDD 100% no DRE enquanto ficarem abertas); 4 Pix CMPC sem par (2.848, 380, 210, 616 — a última classificada como receita avulsa pelo usuário); taxa da maquininha só com o relatório de vendas da InfinityPay (senão entra pelo líquido); linha Evandro −80 (04/09) classificada em 2.1.1.001 (passivo) por engano; classificação em massa das ~1.500 linhas pendentes (a maioria já com sugestão).
- **Decisão de corte (05/09/2026)**: **01/09/2026**. Antes disso, nada de relançar NF de compra — o histórico jan–ago/2026 é tratado só pela classificação na conciliação (as NFs que a Daniela já tinha lançado ficam; casaram automaticamente). A partir de setembro: toda compra com NF passa pela Daniela (estoque + AP + baixa com data/conta reais) e a conciliação casa; compra no débito sem NF vira despesa direto na conciliação. Regra "uma linha, uma origem" (nunca NF + classificação pra mesma compra). Rotina semanal: lançar NFs → importar OFX (Nubank + InfinityPay; reimportar o mês inteiro é seguro) → Casar → Sugerir → Aplicar → sobras → conferir saldo com o app do banco. Estoque parte do inventário físico (marco zero) previsto p/ ~2 semanas depois.
...
| ~~#102~~ | ~~Concluir migração react-query~~ — ✅ Concluído (jul/2026): auditoria de código achou **56 candidatos reais** (bem mais que a estimativa antiga de ~24-35); migrados em 5 lotes ao longo do mês — alertas operacionais/financeiros/custo BOM, permissões (HR/clientes/usuários/módulos), Cotações completo, checklist de produção, BOM de material e composto, ficha técnica completa (ações + fichas de produtos + receita), calendário/detalhe/checklist/lista/notificações/anexos de eventos, `useConfig`/`useTaxonomy` (cache único compartilhado por ~15 arquivos), detalhe/contatos/unidades/salas de cliente, precificação, usuários, produtos de fornecedor, ordens de produção (BOM + planejamento), ciclos de inventário, cadastro de cliente/evento, editor de propostas (o mais delicado — cálculo de margem/frete/precificação), cotações, matching de item de NF, edição de NF, notas fiscais de compra. No caminho, achado e removido **4 arquivos de código morto** sem nenhuma referência no app: `TechnicalSheetForm.tsx`/`TechnicalSheetWizard.tsx` (substituídos por `RecipeBOMForm`/`CompositeBOMForm`) e `ProposalForm.tsx`/`ProposalComposer.tsx` (substituídos pelo `ProposalEditor.tsx` unificado). Ver memória `react-query-data-fetching-standard` — daqui pra frente, código novo deve seguir esse padrão (`useQuery`/`useQueryClient`), não `useState`+`useEffect`+`supabase` manual. | — |
...
| ~~—~~ | ~~Nome sugerido do PDF da proposta não aparece no diálogo nativo~~ — ✅ Concluído (ago/2026): confirmado em Safari real (sugeria "Coffeelier - Sistema de Gestão"); causa: Safari usa o título da **página principal** no "Salvar como PDF", ignorando o `documentTitle` que o `react-to-print` põe no iframe. Fix em `ProposalPDF.tsx`/`PortalProposalPDF.tsx`: `onBeforePrint` troca `document.title` pro nome do arquivo e `onAfterPrint` restaura (inócuo no Chrome, que já usava o do iframe) | — |
...
| — | **Departamento self-service no cadastro do portal**: hoje o convite de acesso ao portal (`ClientPortalUsers.tsx` → edge `invite-client-user`) não captura departamento — quem escolhe é a equipe interna, manualmente, só na hora de montar a proposta (`ProposalEditor.tsx`). Pedido: a pessoa convidada escolhe (ou cria, se não existir) o próprio departamento no passo de definir a senha (`PortalLogin.tsx`, modo `setpw`), ficando já gravado em `client_users`. Requer: coluna `client_users.department_id` (FK `client_departments`); RPC `SECURITY DEFINER` pra gravar (nunca update direto — client_users não tem policy de self-update hoje, e liberar UPDATE geral deixaria o próprio usuário reescrever `portal_role`/`is_active`); RLS de INSERT em `client_departments` pro portal criar departamento novo escopado ao próprio `current_portal_client_id()` (hoje só SELECT é liberado pro portal). Passo novo na UI de `/portal/login` após definir senha. | Média |
| — | **Custos diretos/indiretos da operação na ficha técnica** (ex.: energia elétrica de equipamento — cafeteira 1500W produzindo 8L/30min no café coado) — mostrar na ficha técnica do produto um bloco de custo adicional ao custo-padrão de insumos, pra precificação final mais assertiva. **Bloqueado por design, não por esforço**: exige primeiro fechar o levantamento real de custos operacionais (lançamento de NFs e demais despesas ainda incompletos) — decisão explícita do usuário de **não** criar o bloco/rateio agora com valores estimados, só depois de ter dado real suficiente pra ser assertivo. Ao retomar: **não** modelar como insumo/material comprável na ficha (geraria `stock_movements` de consumo sem nunca ter entrada de compra — estoque negativo permanente, mesma classe do bug de estoque incremental corrigido em jul/2026); modelar como bloco separado (ex. nova tabela `recipe_bom_direct_costs`: descrição, quantidade, taxa unitária) somado ao `cached_unit_cost` sem tocar em estoque. | Baixa (aguardando dados) |
...
- **Ícones**: `lucide-react`
- **`index.html`: `<html lang="pt-BR" translate="no">` + `<meta name="google" content="notranslate">`** — nunca voltar pra `lang="en"` (padrão do Vite). Incidente 13/09/2026: o Chrome de uma usuária (UI em pt-PT) viu a página "em inglês" e traduziu automaticamente; o Google Tradutor reescreve os nós de texto e quebra o React ("Ônibus e Lançar Nota Fiscal", "Forma de reprodução", campo preso em "Carregando…", "Ocorreu um erro inesperado"). Workaround do lado do usuário: ícone de tradução na barra de endereço → "Mostrar original" / "Nunca traduzir este site".
