...
| #70 | Revisão completa fichas técnicas e ordens de produção — ~90% feito (rollup de peso, custo-padrão vivo, finalização com perdas, fluxo paralelo aposentado, duplicidade Receitas/Fichas Técnicas eliminada jul/2026). Falta só: `ProductionReportsEnhanced.tsx` (`loadCostData`) ainda consulta tabelas antigas `recipes`/`recipe_ingredients` em vez de `recipes_bom`/`recipe_bom_items` — aba "Análise de Custos" fica sempre vazia. Já rastreado como tarefa separada (chip pendente). | Média |
