...
| #102 | Concluir migração react-query: auditoria de código (jul/2026) achou **56 candidatos reais** (bem mais que a estimativa antiga de ~24-35) + 16 write-only (só trocar callback por `invalidateQueries`) + 12 hooks de segurança/monitoramento excluídos por design. **Lote 1 migrado (15 arquivos)**: alertas operacionais/financeiros/custo BOM, permissões (HR/clientes/usuários), Cotações, checklist de produção, BOM de material — **~41 restantes**. Migrar o resto ao tocar em cada área. Ver memória `react-query-data-fetching-standard`. | Média |
