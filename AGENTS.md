# AGENTS.md — Coffeelier Gestão

Guia de contexto para o Codex. Leia este arquivo antes de qualquer tarefa.
...
Via MCP do Supabase (disponível no Codex com o plugin instalado):
```
...
| — | Nome sugerido do PDF da proposta (`documentTitle` do `useReactToPrint`, em `ProposalPDF.tsx`/`PortalProposalPDF.tsx`) não aparece no diálogo nativo de impressão/"Salvar como PDF" em todo ambiente — testado ao vivo (jul/2026) no diálogo do macOS: sugeriu "Codex" em vez de `Proposta_<evento>_<numero>`. Código já seta `document.title` corretamente antes de imprimir; a causa provável é o app/webview usado pra testar (não necessariamente falha em Chrome/Safari "puro" — precisa confirmar em navegador padrão de verdade antes de investir mais). Baixo impacto (renomeio manual ao salvar) | Baixa |
