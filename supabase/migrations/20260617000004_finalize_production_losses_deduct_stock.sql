-- Perdas na finalização passam a BAIXAR o estoque (antes só eram logadas em
-- bom_production_losses). Funciona para perda de INGREDIENTE e de PRODUTO
-- ACABADO/INTERMEDIÁRIO (genérico por material, via process_order_component_consumption).
-- A baixa de produto acabado roda DEPOIS do input do rendimento (senão zeraria).
...
BEGIN
  ...
  FOR v_row IN
    SELECT poi.bom_id, poi.total_yield_quantity, poi.actual_yield_quantity, poi.yield_unit,
           rb.finished_material_id, rb.name AS bom_name
