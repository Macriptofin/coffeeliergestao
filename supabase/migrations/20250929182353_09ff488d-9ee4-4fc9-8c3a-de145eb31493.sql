-- ============================================================================
-- INTEGRAÇÃO PROPOSTAS → PRODUÇÃO (ADITIVA - NÃO DESTRUTIVA)
-- Adiciona novas estruturas sem remover proposal_items existente
-- ============================================================================
...
comment on column public.proposals.proposal_kind is 
  'event_table: gera event_table | bom_sku: gera bom_production_orders';
comment on column public.proposals.auto_generated_event_id is 
  'ID do evento gerado automaticamente (para evitar duplicação)';
comment on column public.proposals.auto_generated_event_table_id is 
  'ID da mesa de evento gerada (para evitar duplicação)';
comment on column public.proposals.auto_generated_bom_order_id is 
  'ID da ordem de produção gerada (para evitar duplicação)';
...
comment on table public.proposal_categories is 
  'Categorias livres dentro das propostas (Salgados, Doces, Bebidas, etc.)';
...
comment on table public.proposal_category_items is 
  'Itens dentro de cada categoria - 3 tipos: produce_finished, pick_resale, support_material';
comment on column public.proposal_category_items.item_kind is 
  'produce_finished: produto acabado com BOM | pick_resale: revenda | support_material: apoio';
comment on constraint one_qty on public.proposal_category_items is 
  'Deve ter qty_per_person OU fixed_qty, nunca ambos';
...
comment on table public.proposal_picklists is 
  'Picklists de separação para itens de revenda e apoio';
comment on table public.proposal_picklist_items is 
  'Itens da picklist (apenas pick_resale e support_material)';
...
comment on view public.vw_proposal_breakdown is 
  'Visão consolidada de todos os itens de propostas (nova estrutura)';
...
begin
  ...
  if v_proposal.proposal_kind = 'event_table' then
    ...
    -- Copiar itens produce_finished para event_table_items
    for v_item in
      select * from public.vw_proposal_breakdown
      where proposal_id = p_proposal_id
        and item_kind = 'produce_finished'
    loop
      insert into public.event_table_items (
        ...
        v_item.planned_qty / v_proposal.number_of_people,
        'proposal'
      );
    end loop;
    
    -- Atualizar proposta com ID gerado
    update public.proposals
    set auto_generated_event_table_id = v_event_table_id,
        updated_at = now()
    where id = p_proposal_id;
  ...
  -- CENÁRIO 2: bom_sku
    ...
    -- Copiar itens produce_finished para bom_production_order_items
    for v_item in
      select * from public.vw_proposal_breakdown
      where proposal_id = p_proposal_id
        and item_kind = 'produce_finished'
    loop
      -- Verificar se material tem BOM (recipes_bom ou composites_bom)
      if exists (
        select 1 from public.recipes_bom 
        where finished_material_id = v_item.material_id
      ) then
        -- Inserir item com BOM de receita
        ...
        select
          v_bom_order_id,
          rb.id,
          ceil(v_item.planned_qty / rb.yield_quantity),
          1
        from public.recipes_bom rb
        where rb.finished_material_id = v_item.material_id
        limit 1;
        
      elsif exists (
        select 1 from public.composites_bom 
        where composite_material_id = v_item.material_id
      ) then
        -- Inserir item com BOM de composto
        insert into public.bom_production_order_items (
          production_order_id,
          bom_id,
          quantity,
        ...
        select
          ...
          v_item.planned_qty,
          1
        from public.composites_bom cb
        where cb.composite_material_id = v_item.material_id
        limit 1;
      end if;
    end loop;
    ...
    -- Atualizar proposta com ID gerado
    update public.proposals
    set auto_generated_bom_order_id = v_bom_order_id,
        updated_at = now()
    where id = p_proposal_id;
  ...
  -- Criar picklist para itens de revenda e apoio (ambos cenários)
  if exists (
    select 1 from public.vw_proposal_breakdown
    where proposal_id = p_proposal_id
      and item_kind in ('pick_resale', 'support_material')
  ) then
    insert into public.proposal_picklists (
      proposal_id,
      status
    ) values (
      p_proposal_id,
      'pending'
    ) returning id into v_picklist_id;
    
    -- Copiar itens para picklist
...
comment on function public.generate_production_from_proposal is 
  'Gera event_table ou bom_production_orders + picklist ao aprovar proposta';

-- 7. FUNÇÃO: FINALIZAR ENTREGA (BAIXAR ESTOQUE DE PICKLIST)
-- ============================================================================
...
begin
  ...
  if not found then
    return jsonb_build_object(
      'status', 'error',
      'message', 'Nenhuma picklist pendente encontrada para esta proposta'
    );
  end if;
  
  -- Dar baixa em estoque de cada item
  for v_item in
    select * from public.proposal_picklist_items
    where picklist_id = v_picklist_id
  loop
    -- Criar movimento de estoque (consumo)
    ...
    set 
      current_quantity = current_quantity - v_item.planned_qty,
  ...
  -- Marcar picklist como fulfilled
...
comment on function public.finalize_proposal_fulfillment is 
  'Finaliza entrega: dá baixa em estoque dos itens da picklist';
...
create policy "Admins and managers can manage proposal_category_items"
  on public.proposal_category_items
  for all
  to authenticated
  using (is_admin_or_manager(auth.uid()))
  with check (is_admin_or_manager(auth.uid()));
...
create policy "Admins and managers can manage proposal_picklist_items"
  on public.proposal_picklist_items
  for all
  to authenticated
  using (is_admin_or_manager(auth.uid()))
  with check (is_admin_or_manager(auth.uid()));

-- 9. TRIGGERS DE ATUALIZAÇÃO
-- ============================================================================
...
-- ============================================================================
-- NOTAS IMPORTANTES:
-- 1. NÃO removemos proposal_items - compatibilidade retroativa mantida
-- 2. create_event_from_proposal() permanece inalterado
-- 3. Nova estrutura coexiste com a antiga
-- 4. UI pode usar AMBAS estruturas conforme necessário
-- ============================================================================