-- Drop all variants of calculate_bom_current_cost
DROP FUNCTION IF EXISTS public.calculate_bom_current_cost CASCADE;

-- Agora criar as funções corrigidas
...
BEGIN
  ...
  IF v_material.purchase_unit IS NULL OR v_material.purchase_unit = '' THEN
    v_issues := v_issues || jsonb_build_array('purchase_unit inválido');
  END IF;
  IF v_material.usage_unit IS NULL OR v_material.usage_unit = '' THEN
    v_issues := v_issues || jsonb_build_array('usage_unit inválido');
  END IF;
  IF v_material.conversion_factor IS NULL OR v_material.conversion_factor <= 0 THEN
    v_issues := v_issues || jsonb_build_array('conversion_factor deve ser > 0');
  END IF;
...
BEGIN
  ...
  IF NOT (v_validation->>'valid')::boolean THEN
    RETURN jsonb_build_object('success', false, 'error', 'Unidades inválidas', 'validation', v_validation);
  END IF;
  ...
  END IF;
  v_current_stock := v_stock.current_quantity; v_current_avg_price := v_stock.average_price;
  ...
  VALUES (p_material_id, 'Entrada', v_quantity_in_usage_unit, v_unit_price_in_usage_unit, p_reference_type, p_reference_id,
    COALESCE(p_notes, '') || format(' | %s %s @ R$%s → %s %s @ R$%s',
