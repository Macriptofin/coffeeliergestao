-- Adicionar campo cost_source para rastrear origem do custo
CREATE TYPE cost_source_type AS ENUM ('purchase', 'production', 'manual');

ALTER TABLE public.stock_items
ADD COLUMN cost_source cost_source_type DEFAULT 'purchase',
ADD COLUMN manual_price boolean DEFAULT false,
ADD COLUMN cost_last_updated_at timestamp with time zone DEFAULT now(),
ADD COLUMN cost_last_updated_by uuid REFERENCES auth.users(id);
...
-- Drop e recriar process_stock_entry_with_conversion
DROP FUNCTION IF EXISTS public.process_stock_entry_with_conversion(uuid, numeric, numeric, text, text, uuid, text);

CREATE FUNCTION public.process_stock_entry_with_conversion(
  p_material_id uuid,
  p_quantity numeric,
...
BEGIN
  SELECT 
    id, name, purchase_unit, usage_unit, conversion_factor, material_type
  INTO v_material
  ...
  IF p_entry_unit NOT IN (v_material.purchase_unit, v_material.usage_unit) THEN
    RETURN jsonb_build_object('success', false, 'error', format('Unidade %s inválida', p_entry_unit));
  END IF;
  ...
  IF NOT FOUND THEN
    INSERT INTO public.stock_items (
      material_id, current_quantity, average_price, total_value,
      cost_source, manual_price
    ) VALUES (
      p_material_id, v_qty_in_usage_unit, v_price_in_usage_unit,
      v_qty_in_usage_unit * v_price_in_usage_unit,
      'purchase', false
    );
  ...
  ELSE
    ...
    v_new_avg_price := (
      (v_current_stock * v_current_avg_price) + 
      (v_qty_in_usage_unit * v_price_in_usage_unit)
    ) / v_new_total_qty;

    UPDATE public.stock_items
    SET 
      current_quantity = v_new_total_qty,
      average_price = v_new_avg_price,
      total_value = v_new_total_qty * v_new_avg_price,
  ...
  INSERT INTO public.stock_movements (
    ...
    p_material_id, 'Entrada', v_qty_in_usage_unit, v_price_in_usage_unit,
    'purchase_invoice', p_supplier_id, 
  ...
  v_result := jsonb_build_object(
    ...
    'entry_quantity', p_quantity,
    'entry_unit', p_entry_unit,
    'entry_unit_price', p_unit_price,
    'converted_quantity', v_qty_in_usage_unit,
    'converted_unit', v_material.usage_unit,
    'converted_unit_price', v_price_in_usage_unit,
    'previous_stock', v_current_stock,
    'previous_avg_price', v_current_avg_price,
    'new_stock', v_new_total_qty,
...
-- Drop e recriar produce_finished_product
DROP FUNCTION IF EXISTS public.produce_finished_product(uuid, numeric);

CREATE FUNCTION public.produce_finished_product(
  p_material_id uuid,
  p_quantity numeric
) RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Material não encontrado ou tipo inválido');
  END IF;

  SELECT * INTO v_bom
  FROM public.recipes_bom
  WHERE finished_material_id = p_material_id AND is_archived = false;
  ...
  LOOP
    ...
    FROM public.stock_items WHERE material_id = v_bom_item.material_id;

    IF v_available_qty IS NULL OR v_available_qty < v_required_qty THEN
      RETURN jsonb_build_object('success', false, 
        'error', format('Estoque insuficiente: %s', v_bom_item.name));
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    UPDATE public.stock_items
    SET 
      current_quantity = current_quantity - v_required_qty,
    ...
    INSERT INTO public.stock_movements (
      ...
      v_bom_item.material_id, 'Consumo Produção', v_required_qty,
      v_bom_item.average_price, 'production', v_bom.id,
      format('Produção de %s', v_material.name)
    );
  ...
  INSERT INTO public.stock_items (
    material_id, current_quantity, average_price, total_value,
    cost_source, manual_price, cost_last_updated_at, cost_last_updated_by
  ) VALUES (
    p_material_id, v_bom.yield_quantity * p_quantity, v_unit_cost,
    v_total_cost, 'production', false, now(), auth.uid()
  )
  ...
  INSERT INTO public.stock_movements (
    ...
    p_material_id, 'Entrada Produção', v_bom.yield_quantity * p_quantity,
    v_unit_cost, 'production', v_bom.id, format('Produção de %s lotes', p_quantity)
  );

  v_result := jsonb_build_object(
    'success', true,
    'material_id', p_material_id,
    'quantity_produced', v_bom.yield_quantity * p_quantity,
...
-- Drop e recriar assemble_composite
DROP FUNCTION IF EXISTS public.assemble_composite(uuid, numeric);

CREATE FUNCTION public.assemble_composite(
  p_material_id uuid,
  p_quantity numeric
) RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  SELECT * INTO v_material
  FROM public.materials
  WHERE id = p_material_id AND material_type = 'composite_product';
  ...
  WHERE composite_material_id = p_material_id AND is_archived = false;
  ...
  LOOP
    ...
    SELECT current_quantity INTO v_available_qty
    FROM public.stock_items WHERE material_id = v_bom_item.component_material_id;
  ...
  LOOP
    ...
    INSERT INTO public.stock_movements (
      ...
      v_bom_item.component_material_id, 'Consumo Montagem', v_required_qty,
      v_bom_item.average_price, 'assembly', v_bom.id
    );
  ...
  INSERT INTO public.stock_items (
    material_id, current_quantity, average_price, total_value,
    cost_source, cost_last_updated_at, cost_last_updated_by
  ) VALUES (
    p_material_id, p_quantity, v_total_cost / p_quantity, v_total_cost,
    'production', now(), auth.uid()
  )
  ON CONFLICT (material_id) DO UPDATE
  SET 
    current_quantity = stock_items.current_quantity + EXCLUDED.current_quantity,
  ...
  INSERT INTO public.stock_movements (
    ...
    p_material_id, 'Entrada Montagem', p_quantity,
    v_total_cost / p_quantity, 'assembly', v_bom.id
  );

  v_result := jsonb_build_object(
    'success', true,
    'material_id', p_material_id,
    'quantity_assembled', p_quantity,
    'total_cost', v_total_cost,
