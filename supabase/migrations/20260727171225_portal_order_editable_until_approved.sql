-- Pedido montado pelo próprio cliente deve continuar editável (voltar e mudar
-- itens/momentos) enquanto não for aprovado — só depois de aprovado (produção
-- já disparada) é que uma mudança precisa virar solicitação pra equipe revisar.
-- Precisa de um marcador explícito de autoria: hoje nada distingue "proposta
-- criada pelo cliente" de "proposta criada pela equipe e enviada ao cliente"
-- (as duas usam portal_created_by do mesmo jeito) — sem isso, abrir edição
-- direta pra 'Enviada' deixaria o cliente reescrever uma proposta que a
-- equipe montou.
ALTER TABLE public.proposals ADD COLUMN created_by_client boolean NOT NULL DEFAULT false;
...
-- create_portal_order: editável enquanto Rascunho OU Enviada (ainda não aprovado),
-- e só se foi o próprio cliente que criou (created_by_client) — nunca reescreve
-- uma proposta que a equipe montou.
