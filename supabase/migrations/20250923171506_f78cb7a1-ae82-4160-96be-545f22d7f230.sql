...
-- 1. Add unique constraint to prevent duplicate roles per user
ALTER TABLE public.user_roles ADD CONSTRAINT user_roles_user_id_unique UNIQUE (user_id);

-- 2. Create security function to check if requester is admin (not the target user)
...
-- 3. Create function to check if any admin exists (for bootstrap)
...
-- 4. Drop existing policies and recreate with stronger security
