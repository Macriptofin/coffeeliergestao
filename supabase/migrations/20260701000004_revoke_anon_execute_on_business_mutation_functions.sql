-- Advisor WARN anon_security_definer_function_executable: 174 funções SECURITY
-- DEFINER executáveis pelo papel `anon` (não autenticado). Como SECURITY
-- DEFINER ignora o RLS das tabelas, uma função de mutação sem checagem interna
-- de auth.uid() é, em tese, chamável por QUALQUER PESSOA NA INTERNET usando só
-- a anon key pública (embutida no bundle do navegador) — sem nunca logar.
--
-- Esta migration revoga o EXECUTE do `anon` (mantendo `authenticated` intacto
-- — o app logado continua funcionando exatamente igual) nas funções que:
--   (a) mutam dados de negócio (estoque/custo/produção/financeiro/materiais);
--   (b) são ferramentas de diagnóstico/migração pontual, fora do fluxo normal;
--   (c) são do Portal, que por modelo já exige login (client_users);
--   (d) são funções de trigger (nem são alcançáveis via API, mas por higiene).
--
-- Deliberadamente NÃO tocamos nos helpers de permissão/máscara (has_role,
-- is_admin_or_manager, mask_*, get_user_email_safe, is_portal_client etc.) —
-- são referenciados dentro de policies RLS de outras tabelas, e revogar sem
-- mapear cada policy que os usa arrisca quebrar acesso legítimo de forma
-- sutil (a policy passaria a dar "permission denied" em vez de avaliar para
-- false). Também não tocamos nas funções de log/rate-limit (log_auth_attempt,
-- check_rate_limit, create_account_lockout etc.) — precisam funcionar mesmo
-- em tentativa de login falha, antes da autenticação.
DO $$
DECLARE
  fn record;
