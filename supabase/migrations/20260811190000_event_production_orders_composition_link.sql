-- Achado real (ago/2026): event_production_orders (a "Ordem de Evento" gerada por
-- momento em generate_production_from_proposal) nunca guardou o vínculo com o
-- proposal_compositions de origem — só event_table_id (1 registro por PROPOSTA
-- inteira, com attendees/data agregados de TODOS os momentos). Resultado: a tela/
-- impressão de cada Ordem de Evento mostrava nº de pessoas e data genéricos da
-- proposta toda (iguais em todos os momentos), sem tipo de evento, sala, nº de
-- pessoas nem horário REAIS daquele momento específico.
ALTER TABLE public.event_production_orders
  ADD COLUMN IF NOT EXISTS composition_id uuid REFERENCES public.proposal_compositions(id);

COMMENT ON COLUMN public.event_production_orders.composition_id IS
  'Momento de origem (proposal_compositions) — dono das características reais do evento: tipo, sala, nº de pessoas, data/horário. event_table_id continua existindo só para o agrupamento por proposta.';
