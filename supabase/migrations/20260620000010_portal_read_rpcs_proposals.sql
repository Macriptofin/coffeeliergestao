-- Leitura do portal (posse por current_portal_client_id; sem expor custo/margem).
...
BEGIN
  ...
  WHERE p.client_id = v_client AND p.status <> 'Rascunho';
  RETURN v_result;
END;
$$;
...
BEGIN
  ...
  WHERE p.id = p_proposal_id AND p.client_id = v_client;
