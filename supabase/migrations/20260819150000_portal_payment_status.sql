-- Portal Fase 2: status de pagamento por pedido.
--
-- A ponte é accounts_receivable.proposal_id (campo que sempre existiu no
-- Contas a Receber e nunca foi usado — 0/42 cobranças vinculadas hoje).
-- Regra de exposição: cobrança VINCULADA a uma proposta aparece no portal
-- para o dono daquele pedido (mesmo escopo por usuário de sempre:
-- client_id + portal_created_by = auth.uid()); cobrança sem vínculo não
-- aparece (não é atribuível a um usuário). Nada interno é exposto
-- (centro de custo, conta contábil, banco ficam de fora).
--
-- 1) get_portal_proposals ganha payment_summary por proposta (flag na lista).
-- 2) get_portal_proposal ganha o bloco payments (detalhe do pedido).
-- 3) get_portal_my_payments (nova): todas as cobranças dos pedidos do
--    usuário, pro bloquinho "Pagamentos" da home.
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'payment_summary', (
      SELECT CASE WHEN count(*) = 0 THEN NULL ELSE jsonb_build_object(
        ...
        'open_amount', COALESCE(sum(ar.remaining_amount) FILTER (WHERE ar.status IN ('Pendente','Parcial','Vencido')), 0),
        'overdue_amount', COALESCE(sum(ar.remaining_amount) FILTER (WHERE ar.status = 'Vencido'), 0),
        'next_due_date', min(ar.due_date) FILTER (WHERE ar.status IN ('Pendente','Parcial','Vencido')),
        'payment_status', CASE
          WHEN count(*) FILTER (WHERE ar.status = 'Vencido' AND ar.remaining_amount > 0) > 0 THEN 'Vencido'
          WHEN count(*) FILTER (WHERE ar.status IN ('Pendente','Parcial')) > 0 THEN 'Em aberto'
          ELSE 'Pago' END
      ) END
...
CREATE OR REPLACE FUNCTION public.get_portal_my_payments()
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'remaining_amount', ar.remaining_amount,
    'status', ar.status,
    'proposal_id', p.id,
  ...
  INTO v_result
  FROM public.accounts_receivable ar
  JOIN public.proposals p ON p.id = ar.proposal_id
  WHERE p.client_id = v_client AND p.portal_created_by = auth.uid();
  RETURN v_result;
END;
$function$;

-- ACL: a criação de função herda EXECUTE pra anon via default privileges do
-- Supabase — revogar explicitamente (armadilha documentada em 20260819130000).
REVOKE ALL ON FUNCTION public.get_portal_my_payments() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_portal_my_payments() TO authenticated;
