-- Correções da revisão adversarial da entrega guarda-chuva (19/ago/2026).
--
-- 1) events.composition_id passa a ON DELETE SET NULL. Motivo: com
--    create_event_from_proposal populando o vínculo (item 2), o fluxo normal
--    de reaprovação (persistAll apaga e recria proposal_compositions) passaria
--    a violar a FK NO ACTION. SET NULL espelha o comportamento já existente de
--    events.proposal_id e destrava também a exclusão de propostas (os eventos
--    ficam órfãos, como sempre ficaram — regra "nunca excluir" continua valendo
--    na UI, que esconde Excluir pra guarda-chuva aprovada).
-- 2) create_event_from_proposal grava events.composition_id (o loop já é por
--    composição; só faltava a coluna no INSERT). Sem isso, a composição-molde
--    de toda guarda-chuva aparecia como "sem evento" no extrato
--    (get_umbrella_progress casa por events.composition_id).
-- 3) add_umbrella_execution: total_amount do evento usava cadeia de fallback
--    diferente da usada no price_per_person da composição (omitia o preço do
--    molde) — proposta sem preço de referência gerava evento com R$ 0,00
--    enquanto o extrato mostrava o valor certo. Unificadas as cadeias.
-- 4) get_portal_proposals: next_execution_date usava current_date do servidor
--    (UTC) — entre 21h e meia-noite BRT a execução de HOJE sumia do "próximo
--    fornecimento" (classe de bug já documentada em src/lib/date-utils.ts).
--    Data "hoje" agora calculada em America/Sao_Paulo.

-- ── 1) FK ──────────────────────────────────────────────────────────────
ALTER TABLE public.events
  DROP CONSTRAINT events_composition_id_fkey,
  ADD CONSTRAINT events_composition_id_fkey
    FOREIGN KEY (composition_id) REFERENCES public.proposal_compositions(id)
    ON DELETE SET NULL;

-- ── 2) create_event_from_proposal com vínculo por composição ───────────
...
-- ── 3) add_umbrella_execution: cadeia de fallback do valor unificada ───
...
-- ── 4) get_portal_proposals: "hoje" no fuso do negócio ────────────────
